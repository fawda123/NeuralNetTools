...
#' 
#' @details
#'  This function plots a neural network as a neural interpretation diagram as in Özesmi and Özesmi (1999). Options to plot without color-coding or shading of weights are also provided.  The default settings plot positive weights between layers as black lines and negative weights as grey lines. Line thickness is in proportion to relative magnitude of each weight. The first layer includes only input variables with nodes labelled arbitrarily as I1 through In.  One through many hidden layers are plotted with each node in each layer lablled as H1 through Hn for n nodes in each layer.  The output layer is plotted last with nodes labeled as O1 through On for n output variables.  Bias nodes connected to the hidden and output layers are also shown.  Neural networks created using \code{\link[RSNNS]{mlp}} do not show bias layers, causing a warning to be returned.
#' 
#' @examples 
#' ## using numeric input
#' 
...
#' 
#' plot.numeric(wts_in, struct = struct)
#' 
#' ## using nnet
#' 
#' library(nnet)
...
#' 
#' garson(mod, 'Y1')
plot.nnet <- function(mod_in, nid = T, all_out = T, all_in = T, bias = T, wts_only = F, rel_rsc = 5, circle_cex = 5, node_labs = T, var_labs = T, x_lab = NULL, y_lab = NULL, line_stag = NULL, cex_val = 1, alpha_val = 1, circle_col = 'lightblue', pos_col = 'black', neg_col = 'grey', bord_col = 'lightblue', max_sp = F, ...){
...
#' @method plot numeric
#' 
plot.numeric <- function(mod_in, struct, nid = T, all_out = T, all_in = T, bias = T, wts_only = F, rel_rsc = 5, circle_cex = 5, node_labs = T, var_labs = T, x_lab = NULL, y_lab = NULL, line_stag = NULL, cex_val = 1, alpha_val = 1, circle_col = 'lightblue', pos_col = 'black', neg_col = 'grey', bord_col = 'lightblue', max_sp = F, ...){
